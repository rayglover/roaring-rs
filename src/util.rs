...
impl Halveable for u64 {
    ...
    fn split(self) -> (u32, u32) { ((self >> u32::BITS) as u32, self as u32) }
    fn join(h1: u32, h2: u32) -> u64 { ((h1 as u64) << u32::BITS as u64) + (h2 as u64) }
}
...
impl Halveable for u16 {
    ...
    fn split(self) -> (u8, u8) { ((self >> u8::BITS) as u8, self as u8) }
    fn join(h1: u8, h2: u8) -> u16 { ((h1 as u16) << u8::BITS as u16) + (h2 as u16) }
}
